...
library HookCallLib {
    ...
    function functionCallHook(address target, bytes memory data, bool strict) internal returns (bool success) {
        bytes4 expectedSelector;
        assembly { expectedSelector := mload(add(data, 0x20)) }

        // Verify that the selector was returned from the function call
        (bool success_, bytes memory result) = FunctionCallLib.functionCallGas(target, 1_000_000, data, strict);
        if (success_) {
